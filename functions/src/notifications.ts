...
export const sendOrderNotification = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
...
async function sendEmailForOrderUpdate(orderData: any, type: string, recipientId: string) {
  try {
    let recipient: any;
    let subject: string;
    let template: string;
        ...
        <html>
          <body style="font-family: Arial, sans-serif; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
              ...
              <p>Hola ${data.recipient.personalInfo?.fullName || 'Repartidor'},</p>
